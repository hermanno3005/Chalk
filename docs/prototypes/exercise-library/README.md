...
> Swift source lives at the `prototype-exercise-library` tag, which is where the
...
Check out the `prototype-exercise-library` tag, open `Chalk/Chalk.xcodeproj` and run. The app is rooted at
