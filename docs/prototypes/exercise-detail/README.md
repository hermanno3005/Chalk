...
> Swift source lives at the `prototype-exercise-detail` tag, which is where the
...
Check out the `prototype-exercise-detail` tag, open `Chalk/Chalk.xcodeproj` and run. The app is rooted at
